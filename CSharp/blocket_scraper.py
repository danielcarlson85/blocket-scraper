...
#search_url =
#"https://www.blocket.se/annonser/hela_sverige/fordon/motorcyklar?cg=1140&f=c"
#search_url =
#"https://www.blocket.se/annonser/hela_sverige/fordon/motorcyklar/ovrigt?cg=1148&plo=1&q=motorcykel"
...
def start(url):
    ...
    filename = page_manager.get_search_word(full_blocket_base_webpage, url)
    saved_products = file_manager.load_products_from_file(filename)
    found_products = page_manager.get_all_products(saved_products, full_blocket_base_webpage, url, filename)
    ...
    #    print(x.name +" " + x.price +" " + x.url +" " + x.location +" " + x.date +" " + x.store +" " + x.year +",")
